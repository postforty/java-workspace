...
public class Ex1906 {
  public static void main(String[] args) {
    ...
    try {
      ...
      byte[] buffer = new byte[1024]; // byte 배열 코드 추가
      ...
      // }
      while((read = fileInputStream.read(buffer)) != -1) { // read() 메서드 호출 시 byte 배열 전달
        fileOutputStream.write(buffer, 0, read); // write() 메서드 호출시 byte 배열 전달
      }
